    ...
    $.getJSON("/articles", function(data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].isSaved) {
          ...
          panelTitle.append(
            "<button data-id='" + data[i]._id + "' class='btn btn-success note-button'>" + "Article Notes" + "</button>"
          );
        ...
        // if it is not saved
          ...
          panelTitle.append(
            "<button data-id='" + data[i]._id + "' class='btn btn-primary save-button'>" + "Save Article" + "</button>"
          );
  ...
  $(document).on("click", ".note-button", function() {
    ...
    $.ajax({
      ...
      .then(function(data) {
        ...
        $("#noteModalBody").empty();
        $("#noteModalLabel").append(
          data.headline +
            "<br> <textarea class='form-control' id='titleinput' rows='2' placeholder='Note Title'></textarea>"
        );
        $("#noteModalBody").append(
          "<textarea class='form-control' id='bodyinput' rows='6' placeholder='Note Body'></textarea>"
        );
